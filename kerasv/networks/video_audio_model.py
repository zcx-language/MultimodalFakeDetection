...
def create_model(video_input_shape=None, audio_input_shape=None):
    ''' Create model

    Args:
        video_input_shape {tuple} : video input shape (time_window, height, width, channel).
        audio_input_shape {tuple} : audio input shape (time_window, ?, ?, ?).
    '''
    
    
    # first input model
    input_rgb = Input(shape=video_input_shape)
    ...
    # second input model
    input_stft = Input(shape=audio_input_shape)
